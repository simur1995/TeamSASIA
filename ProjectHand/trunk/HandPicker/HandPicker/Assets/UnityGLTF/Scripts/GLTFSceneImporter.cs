...
namespace UnityGLTF
{
    ...
    public class GLTFSceneImporter
    {
        ...
        public IEnumerator LoadScene(int sceneIndex = -1, bool isMultithreaded = false, Action<GameObject> onLoadComplete = null)
        {
            ...
            VertexUnityPlayer.SceneLink.Instance.MaterialLoaded();
            VertexUnityPlayer.SceneLink.Instance.MeshLoaded();
        }
        ...
        protected IEnumerator ConstructImageBuffer(GLTF.Schema.Texture texture, int textureIndex)
        {
            ...
            if (_assetCache.ImageStreamCache[sourceId] == null)
            {
                ...
                {
                    if(image.Uri.ToLowerInvariant().EndsWith(".png"))
                        image.Uri = image.Uri.Replace(".png", ".dds");

                    if (image.Uri.ToLowerInvariant().EndsWith(".jpg"))
                        image.Uri = image.Uri.Replace(".jpg", ".dds");
        ...
        protected virtual IEnumerator ConstructUnityTexture(Stream stream, bool markGpuOnly, GLTF.Schema.Image image, int imageCacheIndex)
        {
            ...
            if (stream is MemoryStream)
            {
                using (MemoryStream memoryStream = stream as MemoryStream)
                {
                    ...
                    if(bytes[0] == 'D' && bytes[1] == 'D' && bytes[2] == 'S' && bytes[3] == ' ')
                    {
                        ...
                        TextureFormat format = bytes[22] == 8 ? TextureFormat.DXT1 : TextureFormat.DXT5;
                        ...
                        byte[] dxtBytes = new byte[bytes.Length - ddsHeaderSize];
                        System.Buffer.BlockCopy(bytes, ddsHeaderSize, dxtBytes, 0, bytes.Length - ddsHeaderSize);
            ...
            {
                byte[] ddsBytes = new byte[stream.Length];
                ...
                using (stream)
                {
                    ...
                    stream.Read(ddsBytes, 0, (int)stream.Length);
                }
                ...
                System.Buffer.BlockCopy(ddsBytes, ddsHeaderSize, dxtBytes, 0, ddsBytes.Length - ddsHeaderSize);
        ...
        protected virtual void ConstructMaterial(GLTF.Schema.Material def, int materialIndex)
        {
            ...
            if (def.PbrMetallicRoughness != null && mrMapper != null)
            {
                ...
                if (pbr.BaseColorTexture != null)
                {
                    ...
                    //todo: we disable tint because PC doesn't support it, update this later.
                    mrMapper.BaseColorFactor = Color.white;
            ...
            {
                var tex = mapper.Material.GetTexture(key) as Texture2D;
                if(tex != null)
                {
                    if (tex.format == TextureFormat.DXT1 || tex.format == TextureFormat.DXT5)
                    {
                        var scale = mapper.Material.GetTextureScale(key);
                        scale.y *= -1.0f;
                        mapper.Material.SetTextureScale(key, scale);
                    }
                }
            }
